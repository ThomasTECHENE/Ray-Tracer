...
void Image::Map(int i, const glm::vec3 & c) {
	glm::vec3 cc; // La couleur corrigee par bornage dans un intervalle [0,1]
	S_RVB rvb;

	cc = c;
	cc = glm::clamp(cc,glm::vec3(0,0,0), glm::vec3(1,1,1));
	cc *= 255;
...
void Image::Map(int x, int y, const glm::vec3 & c) {
	Map((y * largeur) + x, c);
}
